...
#' @param assocAlpha Numeric value from 0 to 1 of significance threshold for associations. (Default: \code{0.05})
#' @param browserWinPad Numeric value of the padding size (in base pairs) to extend the plotting region. (Default: \code{1000})
#' @param geneName Character of name of the gene of interest. If specified, the function will plot the region associated with this gene. (Default: \code{NULL})
#' @param plotRegionGR \link[GenomicRanges]{GRanges} of length 1 specifying the region to plot. If provided, geneName is ignored. (Default: \code{NULL})
#' @param CreSignalName Character of name of the differential CRE signal track. For plot labeling purposes only (Default: \code{CRE})
#' @param assembly Character of genome assembly name, e.g., "hg38". Must be one of accepted inputs to \code{assembly} argument to \link[plotgardener]{plotGenomeLabel}. (Default: \code{hg38})
#' @param plotWidth Numeric value of width of the browser plot in inches. (Default: \code{dev.size("in")[1]})
#' @param plotHeight  Numeric value of height of the browser plot in inches. (Default: \code{dev.size("in")[2]})
#' @param mergeGenePromotersDist Maximum distance (in base pairs) for merging promoters of the same gene in plot. (Default: \code{1000})
#' @param sigPlotMaxY Numeric value of maximum value for the CRE differential signal plot (Y-axis). (Default: \code{4})
#' @param assocColorRange Numeric vector of values from 0 to 1 of length 2. These values specify the lower and upper values of DegCre association probabilities for color saturation for arch color. (Default: \code{NULL}. If \code{NULL} will be set to 0 and maximum association probability in input data.)
#' @param lowAssocColor Character color for low saturation point of association probabilities in arches plot. (Default: \code{#88CCEE})
#' @param hiAssocColor Character color for high saturation point of association probabilities in arches plot. (Default: \code{#CC6677})
#' @param signalColor Character color for the CRE differential signal plot. (Default: \code{#DDCC77})
#' @param geneLabelFontsize Numeric of font size (as implemented in \href{\url{https://bioconductor.org/packages/release/bioc/html/plotgardener.html}}{plotgardener}) for gene labels. (Default: \code{8})
#' @param axisFontSize Numeric of font size for axis labels and tick marks. (Default: \code{6})
#' @param panelTitleFontSize Numeric of font size for panel titles. (Default: \code{7})
#' @param geneNameColName Character of name of the column in DegGR metadata that was inputted to \link{runDegCre} that contains gene names to query by \code{geneName}. (Default: \code{NULL}. If omitted, the column name will be guessed, with errors if not.)
#' @param geneHighlightDf \link[S4Vectors]{DataFrame} specifying genes to highlight in the plot as accepted by \link[plotgardener]{plotGenes} argument \code{geneHighlights}.
#' @param dePrioritizeSmallRNA Logical, indicating whether small RNA genes should be deprioritized in plotting. (Default: \code{TRUE})
...
#' The output plot consists of an arches plot made with \link[plotgardener]{plotPairsArches} of DegCre associations colored by association probability.
#' Below is a signal track plot made via \link[plotgardener]{plotSignal} of the data in \code{CreGR} that was inputted to \link{runDegCre}.
#' This plot displays the signed negative log p-value, meaning the \eqn{-log_{10}(p_{CRE})} multiplied by the sign of the the CRE log fold-change.
#' Beneath this panel genomic coordinates via \link[plotgardener]{plotGenomeLabel} and gene models via \link[plotgardener]{plotGenes} are displayed.
#'
#' @return Invisibly, a named list containing:
#' \describe{
#'   \item{plotRegionGR}{\link[GenomicRanges]{GRanges} of the plotted region.}
#'   \item{creSignalPlotGR}{\link[GenomicRanges]{GRanges} of the CRE signal (signed negative log CRE p-value) across the plotted region.}
#'   \item{assocGinter}{\link[InteractionSet]{GInteractions} of the DegCre associations in the plotted region.}
#' }
#' @examples
#' \dontrun{
...
plotBrowserDegCre <- function(degCreResList,
	...
	if(all(c(is.null(geneName),is.null(plotRegionGR)))){
		print("Gene name or plot region GenomicRanges must be provided")
		return(NA)
	}
	...
	if(is.null(geneNameColName)){
		...
		#mcols columns name
		listGrepsX <- lapply(c(1:ncol(degGrMcolsDf)),function(colX){
		...
		if(length(maskFoundGene)>1){
			print("Multiple DegGR mcols columns have matches to gene name.")
			print("Re-run setting geneNameColName to correct column name.")
			return(NA)
		}
		if(length(maskFoundGene)==0){
			print("Cannot find gene name column in DEG GR.")
			print("Re-run setting geneNameColName to correct column name.")
			return(NA)
		}
		else{
	...
	print(paste("Minimum display region size =",plotUnitX,"bp."))
	print("All smaller regions will be expanded.")
	...
	if(length(keepDegMask)==0){
		print("No associations passing assocAlpha found for specified region.")
		return(NA)
	}
	...
	colKeyX <- plotXbegin-(0.06*subPlotWidth)
...
#' to a pseudo-continuous signal track suitable for plotting in plotgardener. The signal is derived 
#' from p-values and, optionally, log-fold change values associated with CREs.
#'
#' @param CreGR A \link[GenomicRanges]{GRanges} object representing CRE data.
#' @param plotRegionGR A \link[GenomicRanges]{GRanges} object specifying the region of interest for plotting.
#' @param useLogFC Logical, indicating whether to consider log-fold change values (Default: \code{TRUE}).
#' @param pValColName Character specifying the column name in CreGR containing p-values (Default: \code{pVal}).
#' @param logFcColName Character specifying the column name in CreGR containing log-fold change values (Default: \code{logFC}).
#' @param creSignalBinRes Numeric value specifying the bin resolution in base pairs for the pseudo-continuous signal (Default: \code{100}).
#'
#' @return A \link[GenomicRanges]{GRanges} object with signal values in metadata column \code{score}, suitable for plotting in \link[plotgardener]{plotSignal}.
#'
#' @details
#' Not exported. This function takes a \link[GenomicRanges]{GRanges} object (\code{CreGR}) representing CRE data, extracts p-values, and, 
#' if specified, log-fold change values. It then converts these values into a signed -log p-value in the \code{signal} metadata column.
#' 
#'
#' @examples
#' \dontrun{
...
#'
#' This function generates a GInteractions object suitable for plotting arches in plotgardener.
#'
...
#' @param plotRegionGR A \link[GenomicRanges]{GRanges} object specifying the region of interest for plotting.
#' @param keepPromCreHits A \link[S4Vectors]{Hits} object containing filtered promoter-CRE associations.
#' @param plotUnit Numeric value specifying the minimum plot unit width in base pairs.
#' @param maskFoundGene Character specifying the index of column for gene name in \code{DegGR}.
#' @param mergeGenePromotersDist Numeric value specifying the distance in base pairs within which promoters of the same gene are merged.
#'
#' @return A \link[InteractionSet]{GInteractions} object suitable for arches plotting in plotgardener.
#'
#' @details
#' This function takes genomic data, including differential expression data (\code{DegGR}), CRE data (\code{CreGR}),
#' a region of interest (\code{plotRegionGR}), TSS-CRE associations (\code{keepPromCreHits}), and other parameters,
#' and creates a GInteractions object suitable for plotting arches in plotgardener.
#'
#' Not exported. The function processes the data to ensure that promoters and CREs are in a suitable format for plotting. It merges
#' promoters within a specified distance (\code{mergeGenePromotersDist}) and adjusts the width of regions to meet the
#' minimum plot unit width (\code{plotUnit}). It also handles cases where promoters and CREs may have been merged or
#' contain duplicate regions, ensuring correct associations.
#' This function is meant to run within \link{plotBrowserDegCre}. It will not run well on unintended inputs.
#'
#' @examples
#' \dontrun{
...
makePlotGInter <- function(DegGR,
	...
	if(length(uniqHashNewTempRefHitsMat)<length(hashNewTempRefHitsMat)){
		listNewTempRefHitsMat <- tapply(c(1:nrow(newTempRefHitsMat)),
...
#'
#' Given a list of DegCre results, (\code{degCreResList}), this function generates a \link[GenomicRanges]{GRanges} object
#' encompassing all associated CRE regions for a specific gene with an associated FDR below a specified threshold.
#'
#' @param degCreResList List of DegCre results.
#' @param geneName Character of the name of the gene for which to retrieve associated CRE regions.
#' @param geneNameColName Character specifying the column name for gene names in \code{DegGR}.
#' @param assocAlpha Numeric value from 0 to 1 specifying the threshold for the association probability FDR (Default: \code{0.05}).
#'
#' @return A \link[GenomicRanges]{GRanges} object representing the genomic region encompassing all associated CREs for the specified gene,
#'   or \code{NA} if no associations below the FDR threshold are found.
#'
#' @details
#' Not exported. This function extracts the relevant components from the input \code{degCreResList} and identifies associations for the
#' specified gene with an  association probability FDR below \code{assocAlpha}. If associations are found, it computes
#' the genomic range encompassing all associated CREs and returns it as a GenomicRanges object. If no associations meet the
...
#' \dontrun{
#' #Get plot region.
#' plotRegionGR <- getDegCrePlotRegionFromGene(degCreResList, 
#'                                             geneName = "ERRFI1", 
#'                                             geneNameColName = "GeneSymb", 
#'                                             assocAlpha = 0.05)
#' }
...
getDegCrePlotRegionFromGene <- function(degCreResList,
	...
	if(length(maskGeneInHits)==0){
		print(paste("No associations passing assocAlpha found for",
			geneName))
		plotRegionGR <- NA
	}
	...
	plotRegionGR <- range(allDegCreGRX,ignore.strand=T)
...
#'
#' Given a PlotGardener plot object, this function calculates the Y coordinate for label placement based on the vertical positioning specified by the 'just' parameter.
#'
#' @param plotgardenerObj A plotgardener plot object.
#' @param just Character specifying the vertical positioning of the label.
#'   Options include "top" (top-aligned), "bottom" (bottom-aligned), and "center" (center-aligned). (Default: \code{center})
#'
#' @return A numeric value representing the Y coordinate in inches for label placement.
#'
#' @details
#' Not exported. This function takes a plotgardener plot object as input and calculates the Y coordinate for label placement based on the vertical positioning specified by the 'just' parameter. 
#' The 'just' parameter determines whether the label should be placed at the top, bottom, or center of the plot object. The function returns the calculated Y coordinate.
#'
#' @examples
#' \dontrun{
#' # Get y label.
