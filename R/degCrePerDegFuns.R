...
#' @param geneNameColName Character value of the name of the column in DegGR (that was inputted to \link{runDegCre}) that contains gene names. If NULL, the function will attempt to automatically find the gene name column. (Default: \code{NULL})
#' @param assocAlpha Numeric value from 0 to 1 specifying the significance threshold for associations. (Default: \code{0.05})
#'
#' @details
#' This function calculates the expected associations per DEG based on DegCre analysis results. It first filters significant associations based on the provided association significance threshold (\code{assocAlpha}) and then computes the expected associations per gene. The function returns a \link[S4Vectors]{DataFrame} with gene-level information, including expected associations, number of associations, and significance thresholds.
#'
#' @return A \link[S4Vectors]{DataFrame} with the all data in the input DegGR with these columns added:
#' \describe{
#'   \item{geneName}{Character values of gene names extracted from \code{geneNameColName} column (or column found if \code{geneNameColName = NULL}) in DegGR.}
#'   \item{expectAssocs}{Numeric values of the expected associations per gene.}
#'   \item{nAssocs}{Integer value of the number of associations passing \code{assocAlpha}  per gene.}
#'   \item{assocAlpha}{Numeric value from 0 to 1 of input \code{assocAlpha}}
#'   \item{degAlpha}{Numeric value from 0 to 1 of the significance threshold for DEGs. Obtained from \code{degCreResList}}
#' }
#'
#' @examples
#' \dontrun{
...
getExpectAssocPerDEG <- function(degCreResList,
	...
	if(is.null(geneNameColName)){
		...
		if(length(maskGeneColName)!=1){
			print("Can not find unique gene column name in DegGR mcols.")
			print("Re-run specifying <geneNameColName>.")
			return(NA)
		}
	}
	else{
	...
	expectAssocsAndNmat <- matrix(unlist(expectAssocsList),ncol=2,byrow=T)
...
#'
#' @param expectAssocPerDegDf \link[S4Vectors]{DataFrame} output of \link{getExpectAssocPerDEG}.
#' @param barOutlineColor Color for the outline of the histogram bars. (Default: \code{#88CCEE})
#' @param barFillColor Fill color for the histogram bars. If \code{NULL}, it will be derived from \code{barOutlineColor} with adjusted transparency.
#' @param extraText Logical, indicating whether additional text information (Details) should be added to the plot.
#'
#' @details
#' This function generates a histogram of the expected number of associations per DEG and optionally adds additional text information to the plot, such as DEG FDR, association FDR, and the fraction of DEGs with at least one association.
#' Plot displays a dashed line a value indicating the median expected associations per DEG.
#'
...
#'
#' # Plot histogram of expected associations per DEG
#' medianExpAssocs <- plotExpectedAssocsPerDeg(expectAssocPerDegDf, barOutlineColor = "blue", extraText = TRUE)
#' }
#'
#' @author Brian S. Roberts
#'
