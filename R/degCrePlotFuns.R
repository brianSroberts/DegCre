...
#'
#' @details
#' This function takes a DegCre results list and plots the bin heuristic statistics against different bin sizes. It also highlights the optimal bin size chosen based on the analysis.
#' The y-axis of the plot is the median KS statistic of all bins versus the global CRE p-value distribution.
#'
...
#'
#' @param degCreResList A list of DegCre results.
#' @param assocProbFDRThresh Numeric value from 0 to 1 specifying the FDR threshold for association probability. (Default: \code{0.05})
#' @param plotQRange Numeric vector of quantile range for plotting (e.g., \code{c(0.25, 0.75)} for interquartile range). (Default: \code{c(0.25, 0.75)})
#' @param hiYLim Numeric value specifying the upper limit of the y-axis. (Default: \code{NULL})
#' @param loYLim Numeric value specifying the lower limit of the y-axis. (Default: \code{NULL})
#' @param qRangeFillColor Color for filling the quantile range polygon. (Default: \code{#88CCEE})
#' @param nullLineColor Color for the null association probability line. (Default: \code{#CC6677})
#' @param plotLabCex Numeric value specifying the character expansion factor for labels. (Default: 0.8)
#' @param plotAxisCex Numeric value specifying the character expansion factor for axis labels. (Default: 0.5)
#'
#' @details
#' This function takes the results of the DegCre analysis, including genomic distances and association probabilities, and creates a plot of association probabilities against binned genomic distances. It highlights the quantile range (e.g., interquartile range) and includes a line for null association probabilities.
#' The top panel shows the number of associations passing \code{assocProbFDRThresh}. The bottom panel shows the
#' median FDR-passing association probability as a black line, with the specified quantile range (defaults to interquartile)
#' plotted as \code{qRangeFillColor} region. The \code{nullLineColor} colored line is the null association probability,
#' that is the association probability for a bin with uniform CRE p-values.
#
#' @return Invisibly, a matrix with these columns:
#' \describe{
#'   \item{binMidDist}{Numeric value of the midpoint distance of the bin (TSS to CRE) in kb.}
#'   \item{q_<\code{plotQRange[1] x 100}>}{Numeric value of lower bound of the highlight region.}
#'   \item{q_50}{Numeric value of plotted line.}
#'   \item{q_<\code{plotQRange[2] x 100}>}{Numeric value of upper bound of the highlight region.}
...
#' \describe{
#'   \item{binAssocDist}{Numeric value representing the distance bin (TSS to CRE) in base pairs.}
#'   \item{nullAssocProb}{Numeric value representing the null association probability of the bin.}
#' }
#'
#' @examples
#' \dontrun{
...
#'
#' @param colorVec Character vector of hexadecimal or named colors to be modified.
#' @param newAlpha Numeric value specifying the new alpha transparency level (0-255) (Default: 80).
#'
#' @details
#' Not exported. This function takes a color or vector of colors in hexadecimal or named colors and modifies their transparency by changing the alpha channel value. It returns the modified color(s) with the updated transparency.
#'
#' @return A character vector of modified colors with adjusted transparency in hexadecimal.
#'
#' @examples
#' \dontrun{
