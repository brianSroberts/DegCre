...
#' @details This is a list with two slots: \code{DegGR} and \code{CreGR}.
#' This data was derived from work by McDowell et al. in which they generated RNA-seq and ChIP-seq data by treating A549 cells with dexamethasone at several time points. Specifically this is RNA-seq and NR3C1 ChIP-seq at four hours versus control.
...
#'
#' @param DegGR A \link[GenomicRanges]{GRanges} object of gene TSSs. Multiple TSSs per gene are allowed.
#' @param DegP A numeric vector of differential expression p-values for genes in \code{DegGR}.
#' @param DegLfc A numeric vector of log fold-change values of differential expression for gene in \code{DegGR}. Required when \code{reqEffectDirConcord = TRUE}. (Default: \code{NULL})
#' @param CreGR A \link[GenomicRanges]{GRanges} object of CRE regions.
#' @param CreP A numeric vector differential signal p-values for regions in \code{CreGR}.
#' @param CreLfc A numeric vector log fold-change values of differential signal for regions in \code{CreGR}. Required when \code{reqEffectDirConcord = TRUE}. (Default: \code{NULL})
#' @param reqEffectDirConcord A logical whether to require concordance between the effect direction between DEG and CRE differential values. (Default: \code{TRUE})
#' @param padjMethod A character value indicating the method for p-value adjustment. Do not change from default under most circumstances. Can be any method name accepted by \link[stats]{p.adjust} (Default: \code{bonferroni})
#' @param maxDist An integer value specifying the maximum distance for probability calculation of TSS to CRE associations. (Default: \code{1e6})
#' @param verbose A logical indicating whether to print messages of step completion and algorithm results. (Default: \code{TRUE})
#' @param smallestTestBinSize An integer value specifying the size (number of elements) of the smallest distance bin to be considered in the optimization algorithm. (Default: \code{100})
#' @param fracMinKsMedianThresh A numeric value between 0 and 1 specifying the optimization criterion for the distance bin size algorithm (See Details). (Default: \code{0.2})
#' @param alphaVal A numeric value between 0 and 1 specifying the alpha value for DEG significance. (Default: \code{0.01})
#' @param binNOverride An integer value specifying the number of elements per distance bin. When specified, overrides distance bin size optimization (Not recommended). (Default: \code{NULL})
#'
#' @details
#' The DegCre algorithm considers experimental data from a perturbation experiment and produces associations between cis-regulatory elements (CREs) and differentially expressed genes (DEGs).
#' The user provides differential expression data such as RNA-seq, and differential regulatory signal data such as ATAC-seq, DNase Hypersensitivity, and ChIP-seq.
#' For RNA-seq analysis, we suggest methods such as \href{https://bioconductor.org/packages/release/bioc/html/DESeq2.html}{DESeq2} or \href{https://bioconductor.org/packages/release/bioc/html/edgeR.html}{edgeR}.
#' For the analysis of differential regulatory data we recommend \href{\url{https://bioconductor.org/packages/release/bioc/html/csaw.html}}{csaw}.
#' As an example experiment, we use data from McDowell et al. (PMID = 30097539) in which A549 cells were treated with dexamethasone and control. RNA-seq and ChIP-seq data were collected at various time points.
#'
#' A complete description of the mathematical basis of the DegCre core algorithms is provided in (CITE Roberts et al).
#' DegCre takes two inputs. The first is a GRanges of p-values and optionally log fold-changes associated with DEG TSSs. 
#' The second input is a GRanges of differential signal p-values and optionally log fold-changes for CRE regions.
#' DegCre generates a /link[S4Vectors]{Hits} object of all associations between DEG TSSs and CREs within \code{maxDist}.
...
#' versus minimization of the deviance of each bin's CRE p-value distribution from the global distribution, seleting an optimal bin size.
#'
...
#' An FDR of the association probability is then estimated. Results are returned in list containing a /link[S4Vectors]{Hits} object and both input GRanges.
#'
#' @return A named list containing:
#' \describe{
#'   \item{degCreHits}{A \link[S4Vectors]{Hits} object with metadata. The \link[S4Vectors]{queryHits} of \code{degCreHits} reference \code{DegGR}. The \link[S4Vectors]{subjectHits} of \code{degCreHits} reference \code{CreGR}}
...
#'   \item{DegGR}{\link[GenomicRanges]{GRanges} of input \code{DegGR} with added metadata columns "pVal", "pAdj",and possibly "logFC" if \code{reqEffectDirConcord==TRUE}. Will overwrite existing metadata with same colnames.}
#'   \item{CreGR}{\link[GenomicRanges]{GRanges} of input \code{CreGR} with added metadata columns "pVal", "pAdj",and possibly "logFC" if \code{reqEffectDirConcord==TRUE}. Will overwrite existing metadata with same colnames.}
#' }
#' The degCreHits /link[S4Vectors]{Hits} object metadata has these columns:
#' \describe{
#'     \item{assocDist}{Integer of distance in base pairs between the TSS and CRE for the association.}
#'     \item{assocProb}{Numeric from 0 to 1 of association probability.}
#'     \item{assocProbFDR}{Numeric from 0 to 1 of False discovery rate of the association probability exceeding distance only null.}
#'     \item{rawAssocProb}{Numeric from 0 to 1 of association probability not adjusted for DEG significance or shorter associations involving this CRE.}
...
#'     \item{binAssocDist}{Integer of the maximum association distance cutoff for the bin containing the association.}
#'     \item{numObs}{Integer number of associations in the distance bin containing the association.}
#'     \item{distBinId}{Integer that uniquely identifies the distance containing the association.}
#' }
#'
#' 
#' @examples
...
#' modDegCreResList <- runDegCre(DegGR=DexNR3C1$DegGR,
#'		                      DegP=DexNR3C1$DegGR$pVal,
#'		                      CreGR=DexNR3C1$CreGR,
#'		                      CreP=DexNR3C1$CreGR$pVal,
#'		                      CreLfc=DexNR3C1$CreGR$logFC,
...
runDegCre <- function(DegGR,
	...
	if(all(c(reqEffectDirConcord,any(c(is.null(DegLfc),is.null(CreLfc)))))){
		print("Log fold changes not given when
	...
	if(verbose){
		print(paste("Hit distance bin n =",pickedBinSize))
	}
	...
	if(length(rawSortHitChunksList[length(rawSortHitChunksList)]) < 
		...
		rawSortHitChunksList[[(length(rawSortHitChunksList)-1)]] <-
			last2RawSortChunkIndices[1:last2MidPoint]
	...
	if(verbose){
		print("Adjusting association probability for distance.")
	}
	...
	if(verbose){
		print("Calculation of assoc probabilities over hit bins complete.")
	}
	...
	if(verbose){
		print("Assoc FDR calculation complete.")
	}
	...
	sortPromCreHitsStatsDf <- data.frame(sortPromCreHitsDf[,1:3],
	...
	if(verbose){
		print("DegCre calculations complete.")
	}
...
#' @param CreP A numeric vector differential signal p-values for regions in \code{CreGR}.
#' @param CreLfc A numeric vector log fold-change values of differential signal for regions in \code{CreGR}. Required when \code{reqEffectDirConcord = TRUE}. (Default: \code{NULL})
#' @param reqEffectDirConcord A logical whether to require concordance between the effect direction between DEG and CRE differential values. (Default: \code{NULL})
#' @param padjMethod A character value indicating the method for p-value adjustment. Do not change from default under most circumstances. Can be any method name accepted by \code{p.adjust()} (Default: \code{bonferroni})
#' @param maxDist An integer value specifying the maximum distance for probability calculation of TSS to CRE associations. (Default: \code{1e6})
#' @param verbose A logical indicating whether to print messages of step completion and algorithm results. (Default: \code{NULL})
#' @param smallestTestBinSize An integer value specifying the size (number of elements) of the smallest distance bin to be considered in the optimization algorithm. (Default: \code{100})
#' @param fracMinKsMedianThresh A numeric value between 0 and 1 specifying the optimization criterion for the distance bin size algorithm (See Details). (Default: \code{0.2})
#' @param testedAlphaVals A numeric vector of DEG alpha values to test (Default: \code{c(0.005,0.01,0.02,0.03,0.05,0.1)}).
#'
#' @return A named list containing:
#' \describe{
#'   \item{alphaPRMat}{A matrix of Precision-Recall Area Under the Curve (AUC) values.}
#'   \item{degCreResListsByAlpha}{Named list of DegCre results lists indexed by the \code{testedAlphaVals}}.
#' }
#'
#' The columns of \code{alphaPRMat} are:
#' \describe{
#'   \item{alphaVal}{Numeric vector of tested DEG alpha value.}
#'   \item{AUC}{Numeric vector of Area under the curve of a Precision-Recall (PR) curve based on associations recovering significant DEGs.}
#'   \item{deltaAUC}{Numeric vector of PR AUC minus the AUC of the no-skill line.}
#'   \item{normDeltaAUC}{Numeric vector of the value of \code{deltaAUC} divided by one minus the no-skill AUC.}
#' }
#'
#' @details
...
#' which generates a Precision-Recall curve based on the recovery rate of DEGs by associations.
#' Various AUCs are calculated as performance metrics. Using the alpha with the highest value of
...
optimizeAlphaDegCre <- function(DegGR,
	...
	listByAlpha <- lapply(testedAlphaVals,function(alphaValX){
		if(verbose){
			print(paste("Testing alpha =",alphaValX))
		}
...
#' @param makePlot Logical indicating whether to generate a plot of the Precision-Recall curve. (Default: \code{TRUE})
#' @param nShuff Integer number of shuffles for no-skill curve. (Default: \code{100})
#' @param alphaVal Numeric from 0 to 1 threshold alpha value of DEG significance. (Default: alpha value from \code{degCreResList})
#' @param nThresh Integer number of threshold values for the Precision-Recall curve. (Default: \code{200})
#'
#' @return Invisibly, a list containing:
#' \describe{
#'   \item{actualTprPpvMat}{A matrix of actual True Positive Rate (TPR) and apparent Positive Predictive Value (PPV).}
#'   \item{shuffTprQMat}{A matrix of shuffled TPR quantiles.}
#'   \item{shuffPpvQMat}{A matrix of shuffled PPV quantiles.}
#'   \item{AUC}{Numeric of the total Area Under the Curve (AUC) for the Precision-Recall curve.}
#'   \item{deltaAUC}{Numeric of the difference in AUC between the actual curve and shuffled curves.}
#'   \item{normDeltaAUC}{Numeric of the normalized difference in AUC.}
#' }
#'
#' @details
#' This function calculates the Precision-Recall curve and AUC based on the provided DegCre results. It also estimates the statistical significance of the AUC by shuffling the associations and calculating AUC for shuffled data.
#' Note that the PR AUCs tend to be small (0.05-0.2). Under the calculation framework, a PR AUC of 1 could only be achieved from DegCre results in which every association involves a significant DEG and has an association probability of 1.
#' This situation will never actually occur but serves as a theoretical optimum for comparison.
#'
#'
#' @examples
#' \dontrun{
...
degCrePRAUC <- function(degCreResList,
	...
	if(length(sortTestedAdjProb)<=nThresh){
		...
		threshAdjProbSet1 <- sortTestedAdjProb[1:floor(nThresh/2)]
...
#'
#' Analyzes the associations between DEG and CRE /link[S4Vectors]{Hits} to determine the optimal distance bin size for further analysis.
#'
#' @param degCreHits A /link[S4Vectors]{Hits} object containing the associations between DEG (Differentially Expressed Genes) and CRE (Cis-Regulatory Element) hits with distances.
#' @param CreP A numeric vector of CRE p-values corresponding to the associations in \code{degCreHits}.
#' @param fracMinKsMedianThresh Numeric value from 0 to 1 of the threshold for minimum Kolmogorov-Smirnov (KS) median range. Determines the range of KS statistics considered for optimal bin size. (Default: \code{0.2})
#' @param smallestTestBinSize Integer minimum number of associations in each test bin. (Default: \code{100})
#' @param verbose Logical indicating whether to display progress messages. (Default: \code{TRUE})
#'
#' @return A list containing:
#' \describe{
#'   \item{pickedBinSize}{The optimal distance bin size selected based on KS statistics.}
#'   \item{crePKsMat}{A matrix of distance bin sizes and their corresponding median KS statistics.}
#' }
#'
#' @details
#' Not exported. This function analyzes the associations between DEG and CRE hits to determine the optimal distance bin size. It uses Kolmogorov-Smirnov (KS) statistics to assess the difference in distribution between CRE p-values for different distance bin sizes versus the global. The function selects the largest bin size that falls within the specified fraction of the KS median range.
#' This function operates within \link{runDegCre} on controlled inputs. It will not run well on unintended inputs.
#'
#' @examples
#' \dontrun{
#' # Example usage:
...
#'
#' # Plot the results:
#' plot(binStatsMatrix[, 1], binStatsMatrix[, 2], type = "l", xlab = "Distance Bin Size", ylab = "Median KS Statistic")
#' }
#'
#' @author Brian S. Roberts
#'
distBinHeuristic <- function(degCreHits,
	...
	if(verbose){
		print(paste("Analyzing",numAssocs,
...
#'
#' Calculates the median Kolmogorov-Smirnov (KS) statistic for different distance bin sizes.
#'
#' @param testBinSizes A vector of integer values specifying the distance bin sizes to be tested.
#' @param allSortHitIndices A vector of sorted hit indices.
#' @param allHitPs A numeric vector of p-values corresponding to the associations.
#'
...
#'
#' @details
#' Not exported. This function calculates the median Kolmogorov-Smirnov (KS) statistic for different distance bin sizes. It splits the associations into bins of specified sizes, calculates the KS statistic for each bin, and returns the median KS statistic for each bin size. The KS statistic measures the maximum difference between two cumulative distribution functions, providing a measure of the difference between CRE p-value distributions within each bin and the global distribution.
#' It is meant to run within \link{distBinHeuristic}. It will not run well on unintended inputs.
#'
#' @examples
#' \dontrun{
#' # Example usage:
...
calcKStestStatMedian <- function(testBinSizes,
    ...
    listKsMeanByBinSize <- lapply(testBinSizes, function(binSizeX){
        sortHitChunksListX <- split(allSortHitIndices, ceiling(seq_along(allSortHitIndices)/(binSizeX)))
        
        # Don't use the last bin for analysis because it is always smaller
        sortHitChunksListX <- sortHitChunksListX[1:(length(sortHitChunksListX)-1)]
        
        chunkKSstats <- unlist(lapply(sortHitChunksListX, function(hitIndicesY){
            hitYCrePs <- allHitPs[hitIndicesY]
            
            ksStatY <- fastKS(testSet=hitYCrePs, testIndices=hitIndicesY, refCumProbs=allHitPCumProb)
            return(ksStatY)
        }))
...
#'
#' This function performs a Kolmogorov-Smirnov (KS) test quickly without calculating the p-value. It measures the maximum difference between cumulative probability distributions of a test set and a reference set.
#'
...
#' @param refCumProbs Numeric vector representing the pre-computed cumulative probabilities of the reference set.
#'
#' @return Numeric value of the KS test statistic.
#'
#' @details
#' Not exported. The function compares the cumulative probability distributions of the specified test set to the pre-computed cumulative probabilities of the reference set. It returns the the KS statistic without calculating the p-value (for computational speed).
#' It is meant to run within \link{calcKStestStatMedian}. It will not run well on unintended inputs.
#'
#' @examples
#' \dontrun{
...
#' @param maxDist Integer value representing the maximum distance allowed for associations. Regions further apart than this threshold will not be associated. (Default: \code{1e6})
#'
#' @return A Hits object containing associations and their distances between the genomic regions represented by DegGR and CreGR.
#'
#' @details
#' This function identifies associations between genomic regions from two GenomicRanges objects (DegGR and CreGR) based on their spatial overlap within a specified maximum distance threshold using \link[GenomicRanges]{findOverlaps}. It calculates the distances with \link[GenomicRanges]{distance} between associated regions and stores them in the metadata of the /link[S4Vectors]{Hits} object.
...
getAssocDistHits <- function(DegGR, CreGR, maxDist = 1e6) {
  hitsPromsToCRE <- GenomicRanges::findOverlaps(DegGR, CreGR, maxgap = maxDist, ignore.strand = TRUE)
...
#'
#' This function calculates enrichment statistics for dependent and independent data based on the provided data frame and parameters.
#'
#' @param hitsWithDistDf A \link[S4Vectors]{DataFrame} derived from a Hits object.
#' @param subHitsIndex Indices of the row subsets of \code{hitsWithDistDf} to analyze.
#' @param dependPadj Numeric vector of adjusted p-values for the dependent data.
...
#'
#' @return A matrix containing calculated statistics for enrichment analysis, including independent p-values, associated probabilities, and the total number of observations.
#'
#' @details
#' Not exported. This function calculates enrichment statistics for dependent and independent data based on provided adjusted p-values for dependent data and p-values for independent data. It computes the associated probabilities, which represent the probability of observing a significant association for each set of data under the specified significance level threshold.
#' the independent variable is DegCre calculations is the CreP and the dependent is the DEG adjusted p-values.
#' It is meant to run within \link{runDegCre}. It will not run well on unintended inputs.
#'
#' @examples
#' \dontrun{
#' # Example usage of the function.
...
#' @author Brian S. Roberts
#'
calcDependIndependEnrichStats <- function(hitsWithDistDf, subHitsIndex, dependPadj, independP, alpha) {
  ...
  AssocProbByPThresh[mapUniqToLog10RoundedAll] <- rep(AssocProbByPThreshUniq, times = mapUniqRepTimes)
...
#'
#' This function corrects association probabilities based on distance bins and reference association probabilities.
#'
...
#' @param refAssocProbs A numeric vector of reference association probabilities. (Default: \code{NULL}, uses \code{assocProbs} if \code{NULL})
#'
#' @return A numeric vector of corrected association probabilities.
#'
#' @details
#' Not exported. This function corrects association probabilities within the same distance bin based on reference association probabilities in lower distance bins. It calculates adjusted association probabilities and reference association probabilities for each distance bin and updates the original association probabilities accordingly.
#' The principle is that for all associations involving a single CRE, those associations to significant DEGs that span the shortest distances should be weighted higher than those that span farther distances. 
#' It is meant to run within \link{runDegCre}. It will not run well on unintended inputs.
#'
#' @examples
#' \dontrun{
  ...
  UniqSubjHitAdjProbs <- unlist(tapply(c(1:length(assocProbs)), INDEX = sortHitsDf[,2], FUN = function(indexX) {
    ...
    for (uniqDBinY in uniqDistBinsX) {
      ...
      if (length(subMaskLowerBinsY) > 0) {  
        ...
        adjRefAssocProbsY <- (adjRefAssocProbsY^2) / (sumLowerProbs + adjRefAssocProbsY)
...
#'
#' Calculates the False Discovery Rate (FDR) for association probabilities within a given distance bin using a binomial distribution approach.
#'
#' @param allDistBinsStatsMat A matrix containing statistics for all distance bins.
#' @param chunkI An integer vector specifying the indices of the current distance bin.
...
#'
#' @details
#' Not exported. This function calculates the FDR for association probabilities within a given distance bin. It uses a binomial distribution approach (via \link[stats]{pbinom}) to estimate the FDR based on the number of significant associations and the total number of associations in the bin. Additionally, it adjusts FDR values for associations with low probabilities or ties in the significance ranks.
#' It is meant to run within \link{runDegCre}. It will not run well on unintended inputs.
#'
#' @examples
#' \dontrun{
...
calcBinomFDRperBin <- function(allDistBinsStatsMat, chunkI, alphaVal) {
  ...
  # The FDRs at the low end of the CreP ranks (most significant) must be adjusted
  # because the number of trials in the hypergeometric test is low for these.
  ...
  if(length(maskAboveNull)==0){
	  ...
	  maskIs10 <- which(crepRanks == rank10NonNull)
	  maskInTop10 <- which(crepRanks <= rank10NonNull)
  
	  maskInTop10NonNull <- intersect(maskAboveNull, maskInTop10)
  
	  # There may be multiple FDRs for maskIs10, pick the lowest
	  lowerEndFDR <- min(binomFDRX[maskIs10])
...
#'
#' Calculates the Area Under the Curve (AUC) for a given set of x and y values using the trapezoidal rule.
#'
